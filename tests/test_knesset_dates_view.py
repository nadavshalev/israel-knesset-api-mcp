...
from origins.knesset.get_knesset_dates_view import get_knesset_dates
...
class TestKnessetNumFilter(unittest.TestCase):
    ...
    def test_single_knesset(self):
        ...
        self.assertEqual(results.items[0].knesset_num, 20)
    ...
    def test_knesset_0(self):
        ...
        self.assertEqual(len(results.items), 1)
        self.assertEqual(len(results.items[0].periods), 1)
        self.assertEqual(results.items[0].name, "מועצת המדינה הזמנית")
...
class TestOutputStructure(unittest.TestCase):
    ...
    def test_top_level_keys(self):
        """Each knesset entry has expected attributes."""
        results = get_knesset_dates(knesset_num=20)
        self.assertEqual(len(results.items), 1)
        entry = results.items[0]
        for attr in ("knesset_num", "name", "is_current", "periods"):
            self.assertTrue(hasattr(entry, attr), f"Missing attribute: {attr}")

    def test_period_keys(self):
        """Each period entry has expected attributes."""
        results = get_knesset_dates(knesset_num=20)
        self.assertGreater(len(results.items[0].periods), 0)
        expected_attrs = ("id", "assembly", "plenum", "start_date", "finish_date", "is_current")
        for period in results.items[0].periods:
            for attr in expected_attrs:
                self.assertTrue(hasattr(period, attr), f"Missing attribute: {attr}")

    def test_periods_are_list(self):
        """periods field should be a list."""
        results = get_knesset_dates(knesset_num=20)
        self.assertIsInstance(results.items[0].periods, list)
...
class TestGrouping(unittest.TestCase):
    ...
    def test_knesset_1_has_multiple_periods(self):
        ...
        self.assertEqual(len(results.items), 1)
        periods = results.items[0].periods
    ...
    def test_knesset_25_periods(self):
        ...
        self.assertEqual(len(results.items), 1)
        self.assertEqual(len(results.items[0].periods), 7)
...
class TestCurrentKnesset(unittest.TestCase):
    ...
    def test_current_knesset_is_25(self):
        ...
        results = get_knesset_dates()
        current = [r for r in results.items if r.is_current]
    ...
    def test_current_knesset_has_current_period(self):
        ...
        results = get_knesset_dates(knesset_num=25)
        current_periods = [p for p in results.items[0].periods if p.is_current]
        self.assertEqual(len(current_periods), 1)

    def test_current_period_has_no_finish_date(self):
        """The current period should have finish_date set to None."""
        results = get_knesset_dates(knesset_num=25)
        current_period = [p for p in results.items[0].periods if p.is_current][0]
        self.assertIsNone(current_period.finish_date)

    def test_past_knesset_not_current(self):
        """Knesset 20 should not be current."""
        results = get_knesset_dates(knesset_num=20)
        self.assertFalse(results.items[0].is_current)
...
class TestDateFormatting(unittest.TestCase):
    ...
    def test_known_dates_knesset_25_plenum_1(self):
        ...
        results = get_knesset_dates(knesset_num=25)
        p1 = [p for p in results.items[0].periods
...
class TestKnownData(unittest.TestCase):
    ...
    def test_knesset_20_name(self):
        ...
        results = get_knesset_dates(knesset_num=20)
        self.assertEqual(results.items[0].name, "העשרים")

    def test_knesset_1_name(self):
        """Knesset 1 should be named 'הראשונה'."""
        results = get_knesset_dates(knesset_num=1)
        self.assertEqual(results.items[0].name, "הראשונה")
