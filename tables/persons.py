...
def _insert_to_db(conn, rows: Iterable[Dict[str, Any]]) -> Tuple[int, Optional[str]]:
    ...
    now = _utc_now_iso()
    sql = (
        f"INSERT OR REPLACE INTO {TABLE_NAME} (PersonID, FirstName, LastName, GenderID, GenderDesc, Email, IsCurrent, LastUpdatedDate, fetched_at) "
        "VALUES (:PersonID, :FirstName, :LastName, :GenderID, :GenderDesc, :Email, :IsCurrent, :LastUpdatedDate, :fetched_at)"
    )
    payload = []
