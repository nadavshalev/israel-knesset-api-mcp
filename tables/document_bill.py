...
ODATA_NAME = "KNS_DocumentBill"
TABLE_NAME = "document_bill_raw"
ENSURE_INDEXES = [
    "CREATE INDEX IF NOT EXISTS idx_documentbill_billid ON document_bill_raw (BillID)",
]
# CSV omitted: the oknesset CSV has empty DocumentBillID for all rows,
# making it unusable as a primary key source.  OData provides proper Ids.
...
def create_table(conn) -> None:
    ...
    cur.execute(
        """
        CREATE TABLE IF NOT EXISTS document_bill_raw (
...
def _insert_to_db(conn, rows: Iterable[Dict[str, Any]]) -> Tuple[int, Optional[str]]:
    ...
    sql = (
        ...
        "(Id, BillID, GroupTypeID, GroupTypeDesc, ApplicationID, ApplicationDesc, FilePath, "
        "LastUpdatedDate, fetched_at) "
        "VALUES (%(Id)s, %(BillID)s, %(GroupTypeID)s, %(GroupTypeDesc)s, %(ApplicationID)s, "
        "%(ApplicationDesc)s, %(FilePath)s, %(LastUpdatedDate)s, %(fetched_at)s) "
        "ON CONFLICT (Id) DO UPDATE SET "
        "BillID=EXCLUDED.BillID, GroupTypeID=EXCLUDED.GroupTypeID, GroupTypeDesc=EXCLUDED.GroupTypeDesc, "
