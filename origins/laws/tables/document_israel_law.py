...
ODATA_NAME = "KNS_DocumentIsraelLaw"
TABLE_NAME = "document_israel_law_raw"
ENSURE_INDEXES = [
    "CREATE INDEX IF NOT EXISTS idx_documentisraellaw_israellawid ON document_israel_law_raw (IsraelLawID)",
]
...
def create_table(conn) -> None:
    ...
    cur.execute(
        """
        CREATE TABLE IF NOT EXISTS document_israel_law_raw (
...
def _insert_to_db(conn, rows: Iterable[Dict[str, Any]]) -> Tuple[int, Optional[str]]:
    ...
    sql = (
        ...
        "(Id, IsraelLawID, GroupTypeID, GroupTypeDesc, ApplicationID, ApplicationDesc, FilePath, "
        "LastUpdatedDate, fetched_at) "
        "VALUES (%(Id)s, %(IsraelLawID)s, %(GroupTypeID)s, %(GroupTypeDesc)s, %(ApplicationID)s, "
        "%(ApplicationDesc)s, %(FilePath)s, %(LastUpdatedDate)s, %(fetched_at)s) "
        "ON CONFLICT (Id) DO UPDATE SET "
        "IsraelLawID=EXCLUDED.IsraelLawID, GroupTypeID=EXCLUDED.GroupTypeID, GroupTypeDesc=EXCLUDED.GroupTypeDesc, "
