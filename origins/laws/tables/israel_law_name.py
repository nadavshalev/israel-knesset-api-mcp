...
ODATA_NAME = "KNS_IsraelLawName"
TABLE_NAME = "israel_law_name_raw"
ENSURE_INDEXES = [
    "CREATE INDEX IF NOT EXISTS idx_israellawname_israellawid ON israel_law_name_raw (IsraelLawID)",
]
...
def create_table(conn) -> None:
    ...
    cur.execute(
        """
        CREATE TABLE IF NOT EXISTS israel_law_name_raw (
...
def _insert_to_db(conn, rows: Iterable[Dict[str, Any]]) -> Tuple[int, Optional[str]]:
    ...
    sql = (
        ...
        "(Id, IsraelLawID, LawID, LawTypeID, Name, LastUpdatedDate, fetched_at) "
        "VALUES (%(Id)s, %(IsraelLawID)s, %(LawID)s, %(LawTypeID)s, %(Name)s, "
        "%(LastUpdatedDate)s, %(fetched_at)s) "
        "ON CONFLICT (Id) DO UPDATE SET "
        "IsraelLawID=EXCLUDED.IsraelLawID, LawID=EXCLUDED.LawID, LawTypeID=EXCLUDED.LawTypeID, "
