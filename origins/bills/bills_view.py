...
def _build_bills_search(*, query, knesset_num, date, date_to, top_n):
    ...
    if query:
        conditions.append(fuzzy_condition("b.Name"))
...
def bills(
    ...
    if count_by_val:
        if count_by_val == "all":
            ...
            conn.close()
            return BillsResults(total_count=total_count, items=[], counts=[])
    ...
    total_count = check_search_count(cursor, count_sql, params, entity_name="bills")
