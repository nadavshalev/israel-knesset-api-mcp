...
def _build_secondary_laws_search(*, query, knesset_num, date, date_to, top_n):
    ...
    if query:
        conditions.append(fuzzy_condition("s.Name"))
...
def secondary_laws(
    ...
    full_details: Annotated[bool, Field(description="Include regulators, authorizing laws, bindings, documents. Adds significant data per result — use conservatively. Preferred pattern: search first (full_details=False), then re-call with the specific ID for only the items you need detail on.")] = False,
    ...
    total_count = check_search_count(cursor, count_sql, params, entity_name="secondary_laws")
